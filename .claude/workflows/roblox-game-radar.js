...
const COMMON = `You are helping a SOLO developer decide what Roblox game to build next for a growing portfolio (they already shipped a 500-level procedural maze obby with DataStore saves). Today is 2026-09-04; focus on the LAST 7 DAYS. Use real web research: FIRST call ToolSearch with query "select:WebSearch,WebFetch" to load the tools, then run several WebSearch queries and WebFetch the most promising result URLs. Be concrete, current, and cite source URLs. Your final output IS the structured data (not a message).`
